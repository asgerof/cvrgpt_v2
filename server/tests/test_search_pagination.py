...
def test_pagination(monkeypatch):
    ...
    response = client.get("/v1/search?q=ab&limit=5&offset=0", headers=HDR)
    assert response.status_code == 200
    a = response.json()
    assert a["limit"] == 5 and a["offset"] == 0
    if a["next_offset"] is not None:
        b = client.get(f"/v1/search?q=ab&limit=5&offset={a['next_offset']}", headers=HDR).json()
        assert b["offset"] == a["next_offset"]


def test_limit_cap(monkeypatch):
    monkeypatch.setenv("API_KEY", "secret")
    r = client.get("/v1/search?q=ab&limit=999", headers=HDR)
