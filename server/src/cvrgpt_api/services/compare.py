...
def compare_accounts_snapshots(
    current: Optional[AccountsSnapshot], previous: Optional[AccountsSnapshot]
) -> Dict[str, Any]:
    """Compare two AccountsSnapshot objects and return key changes with narrative."""
    if not current or not previous:
        return {
            "current_period": getattr(current.period, "year", None)
            if current and current.period
            else None,
            "previous_period": getattr(previous.period, "year", None)
    ...
    prev_period = (
        str(previous.period.year) if previous.period and previous.period.year else "previous"
    )
    ...
    if key_changes:
        ...
        for change in top_changes:
            if (
                change.percentage_change and abs(change.percentage_change) > 1
            ):  # Only mention >1% changes
    ...
    narrative = (
        f"Comparing {prev_period} to {curr_period}: " + "; ".join(narrative_parts)
        if narrative_parts
...
def narrate_compare(comp: Dict[str, Any]) -> str:
    ...
    parts = []
    parts.append(
        f"Margin {fmt_pct(curr.get('margin'))} ({('+' if (delta.get('margin') or 0) >= 0 else '')}{fmt_pct(delta.get('margin'))} YoY)"
    )
    parts.append(
        f"Solvency {fmt_pct(curr.get('solvency'))} ({('+' if (delta.get('solvency') or 0) >= 0 else '')}{fmt_pct(delta.get('solvency'))} YoY)"
    )
    parts.append(
        f"Liquidity {fmt_ratio(curr.get('liquidity'))} ({('+' if (delta.get('liquidity') or 0) >= 0 else '')}{fmt_ratio(delta.get('liquidity'))} YoY)"
    )
    return "; ".join(parts) + "."
